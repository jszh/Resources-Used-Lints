...
class ConsultedFormatLint extends DartLintRule {
  ...
  static final RegExp _consultedExpectedRegExp = RegExp(
    r'//\s*CONSULTED\((?=[^()]*\w)[^()\s](?:[^()]*[^()\s])?\):\s+.+',
  );

  const ConsultedFormatLint() : super(code: _code);
  ...
  void _visitCompilationUnit(
    ...
    // Process comments to find CONSULTED declarations and validate them
    for (int i = 0; i < allComments.length; i++) {
      var comment = allComments[i];
      var content = comment.lexeme.trim();

      if (_consultedRegExp.hasMatch(content)) {
        _validateConsultedDeclaration(comment, allComments, i, reporter);
      }
    }
  }

  void _validateConsultedDeclaration(
    ...
    if (!_consultedExpectedRegExp.hasMatch(content)) {
      reporter.atOffset(
        errorCode: _code,
